...
It is a good idea to try and follow the coding conventions laid out in `PEP 8`_.
...
``<root>/gencopy_template.py`` to ``<root>/gencopy.py``, where ``<root>`` is the
root of the ``libtemplate`` repository. Then open up the file
``<root>/gencopy.py``; edit the variables ``new_lib_name_for_imports``,
``new_lib_name_for_docs``, ``author_name``, ``copyright_year``, ``email``,
``lib_description``, ``path_to_new_repo_root``, and ``target_git_platform``; and
...
``new_lib_name_for_imports``, ``author_name``, ``copyright_year``, ``email``,
and ``lib_description`` respectively.
